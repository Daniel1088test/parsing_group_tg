...
async def fetch_channels():
    ...
    try:
        # Convert synchronous ORM query to async using sync_to_async
        get_channels = sync_to_async(lambda: list(Channel.objects.filter(is_active=True)))
...
async def fetch_messages(channel, client):
    ...
    try:
        ...
        get_last_message = sync_to_async(lambda: Message.objects.filter(channel=channel).order_by('-created_at').first())
...
async def save_message(message, channel, session):
    ...
    try:
        ...
        message_exists = sync_to_async(lambda: Message.objects.filter(telegram_message_id=str(message.id), channel=channel).exists())
        if await message_exists():
        ...
        # Save the message
        save_db_message = sync_to_async(lambda: db_message.save())
        await save_db_message()
...
async def main_parser():
    ...
    try:
        ...
        try:
            get_sessions = sync_to_async(lambda: list(TelegramSession.objects.filter(is_active=True)))
        ...
        for channel in channels:
            ...
            # Wrap synchronous relation access in sync_to_async
            get_channel_session = sync_to_async(lambda c=channel: c.session if hasattr(c, 'session') else None)
            channel_session = await get_channel_session()
