...
# add a command to force parse a specific channel
@router.message(Command("forceparse"), F.from_user.id == ADMIN_ID)
async def cmd_force_parse(message: types.Message):
    """Force parse a specific channel to test if parsing works"""
    try:
        args = message.text.split(maxsplit=1)
        if len(args) > 1:
            channel_identifier = args[1].strip()
            
            await message.answer(f"Starting test parsing for: {channel_identifier}")
            
            # Import necessary functions
            from tg_bot.telethon_worker import initialize_client, get_channel_messages
            
            # Initialize a temporary client
            client, me = await initialize_client()
            if not client:
                await message.answer("❌ Failed to initialize Telethon client")
                return
                
            try:
                # Try to get messages
                await message.answer("⏳ Fetching messages...")
                messages, channel = await get_channel_messages(client, channel_identifier)
                
                if not messages:
                    await message.answer(f"❌ No messages found for {channel_identifier}")
                    return
                    
                await message.answer(f"✅ Successfully fetched {len(messages)} messages from {getattr(channel, 'title', channel_identifier)}")
                
                # Process the first message as a sample
                if messages:
                    sample_message = messages[0]
                    
                    # Create a queue and save the message
                    from multiprocessing import Queue
                    test_queue = Queue()
                    
                    # Import save_message_to_data function
                    from tg_bot.telethon_worker import save_message_to_data
                    
                    # Save message
                    await message.answer("⏳ Saving sample message to database...")
                    await save_message_to_data(sample_message, channel, test_queue, None, client, None)
                    
                    # Check if message was queued
                    if not test_queue.empty():
                        queue_item = test_queue.get()
                        if queue_item and 'message_data' in queue_item:
                            message_data = queue_item['message_data']
                            await message.answer(f"✅ Message saved and queued successfully!\n\nMessage ID: {message_data['message_id']}\nChannel: {message_data['channel_name']}\nLink: {message_data['link']}")
                        else:
                            await message.answer("❌ Message was queued but with incorrect format")
                    else:
                        await message.answer("❌ Message failed to be queued")
                    
            except Exception as e:
                import traceback
                error_text = f"❌ Error during test parsing: {e}\n{traceback.format_exc()[:1000]}"
            ...
            await message.answer(
                "Please provide a channel identifier. Examples:\n"
                "1. /forceparse https://t.me/example\n"
                "2. /forceparse @example\n"
                "3. /forceparse example"
            )
